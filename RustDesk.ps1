...
$rustdesk_cfg="=0nI9UVMk5mQrFFexJTQHFWb5IDNx80KxI2NrpVUs52KIlldXhFS1p1a4czVmxmI6ISeltmIsIiI6ISawFmIsIyZy9mLlJXa3lXbuc2bhBXayRnbiojI5FGblJnIsIyZy9mLlJXa3lXbuc2bhBXayRnbiojI0N3boJye"
...
function getLatest()
{
    ...
    catch
    {
        ...
        # Current example link: https://github.com/rustdesk/rustdesk/releases/download/1.2.6/rustdesk-1.2.6-x86_64.msi
    $Downloadlink = ($HTML.Links | Where {$_.href -match '(.)+\/rustdesk\/rustdesk\/releases\/download\/\d{1}.\d{1,2}.\d{1,2}(.{0,3})\/rustdesk(.)+x86_64.msi'} | select -first 1).href

    # bugfix - sometimes you need to replace "about:"
    $Downloadlink = $Downloadlink.Replace('about:', 'https://github.com')

    $Version = "unknown"
    if ($Downloadlink -match './rustdesk/rustdesk/releases/download/(?<content>.*)/rustdesk-(.)+x86_64.msi')
    {
